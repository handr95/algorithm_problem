...
public class TopOfHanoi {
    public int[][] topOfHanoi(int n) {
        List<int[]> path = new ArrayList<>();
        move(n, 1, 3, 2, path);

        int[][] result = new int[path.size()][n];
        int i = 0;
        for (int[] root : path) {
            result[i] = root;
            i++;
    ...
    public void move(int n, int start, int end, int aux, List<int[]> path) {
        if (n == 1) {
            path.add(new int[]{start, end});
        } else {
            move(n - 1, start, aux, end, path);
            path.add(new int[]{start, end});
            move(n - 1, aux, end, start, path);
        }
    }
}
