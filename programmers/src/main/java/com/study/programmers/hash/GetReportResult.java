...
public class GetReportResult {
    public int[] getReportResult(String[] id_list, String[] report, int k) {
        ...
        Map<String, Set<String>> reportInfoMap = new HashMap<>();
        for (String rep : report) {
            String[] reportInfo = rep.split(" ");
            Set<String> reportUserList = reportInfoMap.getOrDefault(reportInfo[1], new HashSet<>());
            reportUserList.add(reportInfo[0]);
            reportInfoMap.put(reportInfo[1], reportUserList);
        }


        int[] reportResult = new int[id_list.length];
        for (Set<String> reporterList : reportInfoMap.values()) {
            if (reporterList.size() >= k) {
                for (String reporter : reporterList) {
                    reportResult[idIndexMap.get(reporter)] ++;
                }
            }
        }
