 ...
 * this character's attack and defense levels. More formally, a character i is said to be weak
 * if there exists another character j where attackj > attacki and defensej > defensei.
 *
...
public class NumberOfWeakCharacters {
    ...
    public int numberOfWeakCharacters(int[][] properties) {
        // 1차원 배열 내림차순 정렬 -> 동일하다면 2차원 배열 오름차순
        Arrays.sort(properties, (o1, o2) -> o1[0]!=o2[0] ? o2[0]-o1[0] : o1[1]-o2[1]);
        int result = 0;
        int bestDefense = 0;
        for (int[] property : properties) {
            if (bestDefense <= property[1]) {
                bestDefense = property[1];
                continue;
            }
