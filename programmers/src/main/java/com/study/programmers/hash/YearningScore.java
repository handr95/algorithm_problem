...
public class YearningScore {
    public int[] solution(String[] name, int[] yearning, String[][] photo) {
        int[] resultScore = new int[photo.length];
        ...
        for (String[] photoList : photo) {
            int score = 0;
            for (String photoItem : photoList) {
                score += nameScore.getOrDefault(photoItem, 0);
            }
            resultScore[index++] = score;
        }
        return resultScore;
    }

    public int[] solutionUsingLambda(String[] name, int[] yearning, String[][] photo) {
